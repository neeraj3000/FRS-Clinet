...
const StudentDashboard = () => {
  ...
  // Transform attendance data for enhanced heatmap
  const transformAttendanceForHeatmap = (attendance) => {
    ...
    Object.entries(attendance).forEach(([subject, data]) => {
      ...
      data.attendance.forEach(entry => {
        if (!heatmapData[entry.date]) {
          heatmapData[entry.date] = {};
        }
        heatmapData[entry.date][subject] = {
          status: entry.status,
      ...
      if (data.consolidated_attendance) {
        data.consolidated_attendance.forEach(consolidated => {
          consolidated.dates.forEach(date => {
            if (!heatmapData[date]) {
              heatmapData[date] = {};
            }
            heatmapData[date][subject] = {
              type: 'consolidated',
              periods: consolidated.number_of_periods,
