...
function Registrations() {
  ...
  const fetchFilteredData = async () => {
    axios
      .get("http://127.0.0.1:8000/admin/filtered-data", {
        params: {
          batch: selectedBatch,
  ...
  const handleDelete = async (studentId) => {
    try {
      await axios.delete(
        `http://127.0.0.1:8000/admin/delete-student/${studentId}`
      );
  ...
  const handleEdit = (id) => {
    ...
    // Add navigation or form opening logic here
  };
              ...
              <TableRow key={index}>
                ...
                <TableCell>
                  {item.embeddings ? (
                    ...
                    <CancelIcon color="error" /> // Show CancelIcon if embeddings are missing
                  )}
                </TableCell>
                <TableCell>
                  <IconButton
