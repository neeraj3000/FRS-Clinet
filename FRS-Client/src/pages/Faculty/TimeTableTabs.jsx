...
import FacultyTimeTableManager from "./FacultyTimeTableManager";
...
const TimeTableTabs = () => {
  ...
  const handleChange = (event, newValue) => {
    setValue(newValue);
    // navigate(`/faculty/timetable/${years[newValue].value}`); // Update the URL based on the selected tab
  };
      ...
      {years.map((item, index) => {
        return (
          <CustomTabPanel key={index} value={value} index={index}>
            <FacultyTimeTableManager year={item.value}></FacultyTimeTableManager>
          </CustomTabPanel>
        );
      })}
    </Box>
